...
<title>Quadern CCPAE · Gabalda</title>
<style>
:root{--earth:#3d2b1f;--soil:#6b4226;--clay:#a0633a;--sand:#d4a574;--wheat:#f0d9b5;--cream:#faf6f0;--vine:#2d5016;--leaf:#4a7c25;--grape:#6b2d6b;--white:#fff;--danger:#c0392b;--warn:#e67e22;}
...
/* CONFIG BANNER */
#cfg-banner{background:#fff3cd;border-bottom:2px solid #ffc107;padding:12px 20px;font-size:0.85rem;color:#856404;display:flex;align-items:center;gap:10px;flex-wrap:wrap;}
#cfg-banner.hidden{display:none;}
#cfg-banner input{flex:1;min-width:200px;padding:6px 10px;border:1px solid #ffc107;border-radius:6px;font-size:0.85rem;}
#cfg-banner button{padding:6px 14px;background:#856404;color:white;border:none;border-radius:6px;cursor:pointer;font-size:0.85rem;white-space:nowrap;}
...
.lbtn{width:100%;padding:15px;background:linear-gradient(135deg,#a0633a,#d4a574);border:none;border-radius:10px;color:#3d2b1f;font-size:1rem;font-weight:600;cursor:pointer;transition:transform 0.15s;}
.lbtn:hover:not(:disabled){transform:translateY(-2px);}
.lbtn:disabled{opacity:0.6;cursor:not-allowed;}
.lmsg{margin-top:14px;color:rgba(240,217,181,0.8);font-size:0.85rem;min-height:20px;}
...
.nav{background:#fff;border-bottom:2px solid var(--wheat);display:flex;overflow-x:auto;padding:0 16px;gap:2px;scrollbar-width:none;}
.nav::-webkit-scrollbar{display:none;}
.ntab{padding:14px 16px;font-size:0.82rem;font-weight:500;color:var(--clay);cursor:pointer;white-space:nowrap;border-bottom:3px solid transparent;background:none;border-left:none;border-right:none;border-top:none;transition:all 0.2s;}
...
.btn{padding:11px 22px;border-radius:8px;font-size:0.9rem;font-weight:600;cursor:pointer;border:none;display:inline-flex;align-items:center;gap:6px;transition:transform 0.15s;}
...
.tb{padding:8px 16px;border-radius:20px;border:1.5px solid var(--wheat);background:var(--cream);color:var(--soil);font-size:0.82rem;font-weight:500;cursor:pointer;transition:all 0.2s;}
...
.rdl{position:absolute;top:10px;right:12px;background:none;border:none;color:var(--clay);cursor:pointer;font-size:1rem;padding:2px 6px;border-radius:4px;}
...
.pb{padding:8px 10px;border:1.5px solid var(--wheat);border-radius:8px;background:var(--cream);cursor:pointer;font-size:0.8rem;color:var(--earth);text-align:left;line-height:1.3;transition:all 0.15s;}
...
<!-- BANNER CONFIGURACIÓ -->
<div id="cfg-banner">
  ⚙️ <strong>Primer cop:</strong> Enganxa la URL del teu Apps Script per connectar amb Google Sheets:
  <input type="text" id="cfg-url" placeholder="https://script.google.com/macros/s/.../exec" />
  <button onclick="saveApiUrl()">💾 Guardar i connectar</button>
</div>
...
<div id="app">
  ...
  <div class="con hidden" id="t-nou">
    <div class="card">
      ...
      <div class="fld" id="wp">
        <label>Parcel·les <span class="req">*</span></label>
        <div class="pg" id="pg"></div>
        <div id="ps2" style="font-size:0.8rem;color:var(--vine);margin-top:4px;font-weight:600;"></div>
      </div>
      <div class="fr" id="wc">
        <div class="fld"><label>Cultiu</label><div id="c-cu"></div></div>
        <div class="fld"><label>Superfície (ha)</label><input type="number" id="fsu" step="0.01"/></div>
      </div>
      ...
      <div id="s-treball">
        ...
        <div class="fr">
          <div class="fld"><label>Quantitat sembrada (kg/ha)</label><input type="number" id="fqs" step="0.01"/></div>
          <div class="fld"><label>Observacions</label><input type="text" id="fob"/></div>
        </div>
      </div>

      <!-- FITO -->
      <div id="s-fito" class="hidden">
        <hr class="dv2">
        <div class="fr fr1"><div class="fld"><label>Plaga o malaltia <span class="req">*</span></label><div id="c-pl"></div></div></div>
        <div class="fr">
          <div class="fld"><label>Producte (nom + m.a.) <span class="req">*</span></label><div id="c-fp"></div></div>
          <div class="fld"><label>Núm. registre</label><div id="c-fr"></div></div>
        </div>
        <div class="fr fr3">
          ...
          <div class="fld"><label>Eficàcia (0-3)</label>
            <select class="ps" id="ffe"><option value="">—</option><option value="0">0-Nul·la</option><option value="1">1-Dolenta</option><option value="2">2-Regular</option><option value="3">3-Bona</option></select>
          </div>
        </div>
        <div class="fr">
          <div class="fld"><label>Aplicador</label>
            <select class="ps" id="ffa"><option value="A-1 Eloi Gabalda Ferrè">A-1 · Eloi</option><option value="A-2 Valero Gabalda Bes">A-2 · Valero</option></select>
          </div>
          <div class="fld"><label>Màquina</label>
            <select class="ps" id="ffm"><option value="D-1 Ensofradora">D-1 · Ensofradora</option><option value="D-2 Atomisador (503398)">D-2 · Atomisador</option></select>
          </div>
        </div>
      </div>

      <!-- ADOB -->
      <div id="s-adob" class="hidden">
        <hr class="dv2">
        <div class="fr">
          <div class="fld"><label>Tipus fertilitzant <span class="req">*</span></label><div id="c-at"></div></div>
          <div class="fld"><label>Nom comercial</label><div id="c-an"></div></div>
        </div>
        <div class="fr fr3">
          <div class="fld"><label>Composició / NPK</label><input type="text" id="fac" placeholder="ex. 5-3-3"/></div>
          <div class="fld"><label>Conc. N (kg N/t)</label><input type="number" id="fan" step="0.01"/></div>
          <div class="fld"><label>Quantitat (t o m³)</label><input type="number" id="faq" step="0.001"/></div>
        </div>
        <div class="fr">
          <div class="fld"><label>Origen</label><div id="c-ao"></div></div>
          <div class="fld"><label>Aplicador</label>
            <select class="ps" id="faa"><option value="F-1 Eloi Gabalda Ferrè">F-1 · Eloi</option><option value="F-2 Valero Gabalda Bes">F-2 · Valero</option></select>
          </div>
        </div>
      ...
      <!-- COLLITA -->
      <div id="s-collita" class="hidden">
        <hr class="dv2">
        <div class="fr">
          <div class="fld"><label>Cultiu recol·lectat <span class="req">*</span></label><div id="c-cc"></div></div>
          <div class="fld"><label>Qualificació</label>
            <select class="ps" id="fcq"><option value="CON">CON – En conversió</option><option value="ECO">ECO – Ecològic</option><option value="CVL">CVL – Convencional</option></select>
          </div>
        </div>
        <div class="fr fr3">
          <div class="fld"><label>Venda (kg)</label><input type="number" id="fcv" step="0.1"/></div>
          <div class="fld"><label>Autoconsum (kg)</label><input type="number" id="fca" step="0.1"/></div>
          <div class="fld"><label>Total (kg)</label><input type="number" id="fct" step="0.1" readonly style="background:#f0f7ec;cursor:not-allowed;"/></div>
        </div>
      </div>

      <!-- COMPRA -->
      <div id="s-compra" class="hidden">
        <hr class="dv2">
        <div class="fr">
          <div class="fld"><label>Producte <span class="req">*</span></label><div id="c-cp"></div></div>
          <div class="fld"><label>Qualificació</label>
            <select class="ps" id="fcpq"><option value="ECO">ECO</option><option value="CVL">CVL</option><option value="AUT">AUT</option></select>
          </div>
        </div>
        <div class="fr fr3">
          <div class="fld"><label>Quantitat (kg o L)</label><input type="text" id="fcpqt"/></div>
          <div class="fld"><label>Proveïdor</label><div id="c-cpv"></div></div>
          <div class="fld"><label>Núm. albarà</label><input type="text" id="fcpa"/></div>
        </div>
      </div>

      <!-- VENDA -->
      <div id="s-venda" class="hidden">
        <hr class="dv2">
        <div class="fr">
          <div class="fld"><label>Producte <span class="req">*</span></label><div id="c-vp"></div></div>
          <div class="fld"><label>Qualificació</label>
            <select class="ps" id="fvq"><option value="CON">CON</option><option value="ECO">ECO</option><option value="CVL">CVL</option></select>
          </div>
        </div>
        <div class="fr fr3">
          <div class="fld"><label>Quantitat (kg)</label><input type="number" id="fvqt" step="0.1"/></div>
          <div class="fld"><label>Núm. albarà</label><input type="text" id="fva"/></div>
          <div class="fld"><label>Client</label><div id="c-vc"></div></div>
        </div>
      </div>

      <!-- ALTRES -->
      <div id="s-altres" class="hidden">
        <hr class="dv2">
  ...
  <div class="con hidden" id="t-hist">
    ...
    <div class="fb">
      ...
      <select class="ps" id="fft" onchange="renderH()" style="max-width:180px;">
        ...
        <option value="treball">Treball camp</option>
        <option value="fito">Fitosanitari</option>
        <option value="adob">Adobat</option>
        <option value="collita">Recol·lecció</option>
        <option value="compra">Compra</option>
        <option value="venda">Venda</option>
        <option value="altres">Incidència</option>
      </select>
    </div>
    <div id="hl"></div>
  </div>
  ...
  <div class="con hidden" id="t-exp">
    <div class="ec">
      <h3>📥 Exportar Excel CCPAE</h3>
      <p>Genera el fitxer Excel oficial per presentar a l'inspector del CCPAE.</p>
      <button class="bw" onclick="exportXl()">⬇️ Descarregar Excel</button>
    </div>
    <div class="card"><div class="ctit">📊 Resum de registres</div><div id="esum"></div></div>
  </div>
</div>
...
<script>
// ── CONFIG ────────────────────────────────────────────────
let API_URL = localStorage.getItem('ccpae_api_url') || '';

function saveApiUrl() {
  const url = document.getElementById('cfg-url').value.trim();
  if (!url || !url.startsWith('https://script.google.com')) {
    toast('URL no vàlida', 'e'); return;
  }
  localStorage.setItem('ccpae_api_url', url);
  API_URL = url;
  document.getElementById('cfg-banner').classList.add('hidden');
  toast('✅ URL guardada! Ara pots entrar.');
}

// Mostrar banner si no hi ha URL
if (!API_URL) {
...
// ── OPCIONS (localStorage + API) ─────────────────────────
function gO(k) { return OPTS[k] || DEFS[k] || []; }
function sO(k, v) {
  if (!gO(k).includes(v)) {
    OPTS[k] = [v, ...(OPTS[k] || DEFS[k] || [])];
    apiPost({ action: 'saveOpts', user: CU, opts: OPTS }).catch(() => {});
  }
}
function dO(k, v) {
  OPTS[k] = gO(k).filter(x => x !== v);
  apiPost({ action: 'saveOpts', user: CU, opts: OPTS }).catch(() => {});
}

// ── API via JSONP (únic mètode que funciona amb Apps Script + CORS) ──
let _cbCount = 0;

function jsonp(params) {
  return new Promise((resolve, reject) => {
    const cbName = '_ccpae_cb_' + (++_cbCount) + '_' + Date.now();
    const timeout = setTimeout(() => {
      delete window[cbName];
      if (script.parentNode) script.parentNode.removeChild(script);
      reject(new Error('Timeout - comprova la URL de l\'Apps Script'));
    }, 15000);
    ...
    script.onerror = () => {
      clearTimeout(timeout);
      delete window[cbName];
      reject(new Error('Error carregant script - URL incorrecta?'));
    };
...
async function apiPost(body) {
  ...
  if (body.data)  params.payload = JSON.stringify(body.data);
  if (body.user)  params.user    = body.user;
  if (body.opts)  params.opts    = JSON.stringify(body.opts);
  if (body.id)    params.id      = String(body.id);
  return jsonp(params);
}
...
async function doLogin() {
  ...
  if (!API_URL) { toast('Primer configura la URL de l\'Apps Script al banner groc', 'w'); return; }

  CU = u;
  const btn = document.getElementById('lbtn');
  btn.disabled = true; btn.textContent = '⏳ Carregant...';
  document.getElementById('lmsg').textContent = 'Connectant amb Google Sheets...';

  try {
    const [rRes, oRes] = await Promise.all([
      apiGet({ action: 'loadRecords', user: u }),
      apiGet({ action: 'loadOpts', user: u })
    ]);
    RECS = rRes.data || [];
    OPTS = oRes.data || {};
    ...
    document.getElementById('lmsg').textContent = '❌ Error de connexió: ' + err.message;
...
function buildPG() {
  ...
  PARC.forEach(p => {
    ...
    b.className = 'pb'; b.dataset.n = p.n;
    b.innerHTML = `<div class="pn">${p.n}</div><div>${p.nom}</div><div style="font-size:0.7rem;color:var(--clay)">${p.sup} ha</div>`;
    b.onclick = () => togP(p.n, b);
    g.appendChild(b);
  });
}
function togP(n, b) {
  const i = SP.indexOf(n);
  if (i >= 0) { SP.splice(i, 1); b.classList.remove('on'); }
  else { SP.push(n); b.classList.add('on'); }
  updPS();
}
function updPS() {
...
function mkC(cid, k, ph) {
  ...
  const dd = document.createElement('div'); dd.className = 'dd'; dd.id = 'cd_' + k;
  ...
  function rf(f) {
    ...
    const t = (f || '').trim();
    if (t && !gO(k).includes(t)) {
      const a = document.createElement('div'); a.className = 'di an'; a.textContent = `➕ Afegir "${t}"`;
      a.onclick = () => { sO(k, t); inp.value = t; dd.style.display = 'none'; toast(`"${t}" guardat ✓`); };
      dd.appendChild(a);
    }
  }
...
async function saveRec() {
  ...
  const base = {
    id: Date.now(), type: CT, data: d,
    dataFinal: document.getElementById('fdf').value || null,
    parcelles: [...SP], cultiu: gcv('cu'),
    superfici: document.getElementById('fsu').value || null,
    user: CU, createdAt: new Date().toISOString()
  };
  let sp = {};

  if (CT === 'treball') {
    const t = gcv('tr'); if (!t) { toast('Indica el treball', 'e'); return; }
    sO('tr', t); if (base.cultiu) sO('cu', base.cultiu);
    sp = { treball: t, quantitatSembrada: document.getElementById('fqs').value || null, observacions: document.getElementById('fob').value || null };
  } else if (CT === 'fito') {
    const pl = gcv('pl'), pr = gcv('fp');
    if (!pl || !pr) { toast('Plaga i producte obligatoris', 'e'); return; }
    sO('pl', pl); sO('fp', pr);
    const rg = gcv('fr'); if (rg) sO('fr', rg);
    sp = { plaga: pl, prod: pr, numRegistre: rg, dosi: document.getElementById('ffd').value || null,
      brouLitres: document.getElementById('ffb').value || null, eficacia: document.getElementById('ffe').value || null,
      aplicador: document.getElementById('ffa').value, maquina: document.getElementById('ffm').value };
  } else if (CT === 'adob') {
    const tp = gcv('at'); if (!tp) { toast('Indica el fertilitzant', 'e'); return; }
    sO('at', tp);
    const nm = gcv('an'); if (nm) sO('an', nm);
    const or = gcv('ao'); if (or) sO('ao', or);
    sp = { tipus: tp, nom: nm, origen: or, composicio: document.getElementById('fac').value || null,
      concN: document.getElementById('fan').value || null, quantitat: document.getElementById('faq').value || null,
      fertirreg: document.getElementById('faf').checked, aplicador: document.getElementById('faa').value };
  } else if (CT === 'collita') {
    const cl = gcv('cc'); if (cl) sO('cc', cl);
    sp = { cultiu: cl, qualificacio: document.getElementById('fcq').value,
      prodVenda: document.getElementById('fcv').value || null, prodAutoconsum: document.getElementById('fca').value || null,
      prodTotal: document.getElementById('fct').value || null };
    base.cultiu = cl;
  } else if (CT === 'compra') {
    const pr = gcv('cp'); if (!pr) { toast('Indica el producte', 'e'); return; }
    sO('cp', pr);
    const pv = gcv('cpv'); if (pv) sO('cpv', pv);
    sp = { prod: pr, prov: pv, qualificacio: document.getElementById('fcpq').value,
      quantitat: document.getElementById('fcpqt').value || null, albara: document.getElementById('fcpa').value || null };
  } else if (CT === 'venda') {
    const pr = gcv('vp'); if (!pr) { toast('Indica el producte', 'e'); return; }
    sO('vp', pr);
    const cl = gcv('vc'); if (cl) sO('vc', cl);
    sp = { prod: pr, client: cl, qualificacio: document.getElementById('fvq').value,
      quantitat: document.getElementById('fvqt').value || null, albara: document.getElementById('fva').value || null };
  } else if (CT === 'altres') {
    const ds = document.getElementById('fal').value.trim();
    if (!ds) { toast('Escriu la incidència', 'e'); return; }
    sp = { descripcio: ds };
  }
  ...
  try {
    const res = await apiPost({ action: 'saveRecord', data: rec });
    if (res.ok) {
      RECS.unshift(rec);
      toast('✅ Registre guardat al Google Sheets!');
      resetF(); renderI();
    } else {
      toast('Error: ' + (res.error || 'desconegut'), 'e');
    }
  } catch(err) {
    toast('Error de xarxa: ' + err.message, 'e');
  }
...
function renderI() {
  const r = RECS;
  const c = {treball:0,fito:0,adob:0,collita:0,compra:0,venda:0};
  r.forEach(x => { if (c[x.type] !== undefined) c[x.type]++; });
  document.getElementById('sg').innerHTML = [
    ['Treballs','🌱',c.treball],['Fitosanitaris','🌿',c.fito],['Adobats','🪣',c.adob],
    ['Recol·leccions','🍇',c.collita],['Compres','🛒',c.compra],['Vendes','💰',c.venda]
  ].map(([l,i,n]) => `<div class="sc"><div style="font-size:1.4rem">${i}</div><div class="sn">${n}</div><div class="sl">${l}</div></div>`).join('');
  const el = document.getElementById('recent');
  const rc = r.slice(0, 8);
...
function rH(r, del) {
  ...
  return `<div class="ri">
    <div class="rd ${DC[r.type]||'dal'}"></div>
    <div class="rm">
      <div class="rda">${dt}${r.dataFinal?' → '+r.dataFinal.split('-').reverse().join('/'):''} · ${r.user}</div>
      <div class="rt">${L[r.type]||r.type}</div>
      <div class="rde">${d}</div>
    </div>
    ${del ? `<button class="rdl" onclick="delR(${r.id})">🗑</button>` : ''}
  </div>`;
}

async function delR(id) {
  if (!confirm('Eliminar?')) return;
  await apiPost({ action: 'deleteRecord', user: CU, id }).catch(() => {});
  RECS = RECS.filter(r => r.id !== id);
  renderH(); renderI(); toast('Eliminat');
}
...
function renderES() {
  ...
  document.getElementById('esum').innerHTML = Object.entries(L).map(([k,l]) =>
    `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--wheat);font-size:0.9rem;"><span>${l}</span><strong style="color:var(--vine)">${r.filter(x=>x.type===k).length}</strong></div>`
  ).join('') + `<div style="padding:10px 0;font-weight:700;">Total: ${r.length} registres</div>`;
}
...
function exportXl() {
  ...
  XLSX.utils.book_append_sheet(wb, ws([
    ['QUADERN D\'EXPLOTACIÓ - PRODUCCIÓ VEGETAL ECOLÒGICA'],[''],
    ['Titular:','Eloi Gabalda Ferrè'],['NIF:','47857445B'],
    ['Núm. Registre CCPAE:','CT/…/P'],['Data obertura:','07/06/2024'],
    ['Exportat:',new Date().toLocaleDateString('ca-ES')]
  ],[50,30]),'0-Portada');

  const t3 = recs.filter(r=>r.type==='treball');
  XLSX.utils.book_append_sheet(wb, ws([
    ['Data o període','Número finca','Superfície (ha)','Cultiu','Treballs del sòl, sembres i altres treballs','Quantitat sembrada (Kg/Ha)','Observacions','Operari'],
    ...t3.map(r=>[r.dataFinal?fd(r.data)+'-'+fd(r.dataFinal):fd(r.data),fn(r.parcelles),r.superfici||'',r.cultiu||'',r.treball||'',r.quantitatSembrada||'',r.observacions||'',r.user])
  ],[20,22,12,26,35,14,25,18]),'3-Treballs camp');

  const t6 = recs.filter(r=>r.type==='fito');
  XLSX.utils.book_append_sheet(wb, ws([
    ['Data tractament','Número finca','Cultiu','Plaga o malaltia','Superfície (ha)','Aplicador','Màquina','L. brou','Producte (nom+m.a.)','Núm. registre','Dosi','Eficàcia (0-3)'],
    ...t6.map(r=>[fd(r.data),fn(r.parcelles),r.cultiu||'',r.plaga||'',r.superfici||'',r.aplicador||'',r.maquina||'',r.brouLitres||'',r.prod||'',r.numRegistre||'',r.dosi||'',r.eficacia||''])
  ],[16,22,22,28,12,22,22,10,30,14,14,10]),'6-Fitosanitaris');

  const t4 = recs.filter(r=>r.type==='adob');
  XLSX.utils.book_append_sheet(wb, ws([
    ['Número finca','Cultiu','Superfície (ha)','Data o període','Tipus fertilitzant','Nom comercial','Composició','Origen','Conc. N (kg N/t)','Quantitat (t/m³)','Kg N aplicats','Dosi kg N/ha','Fertirrigació','Aplicador'],
    ...t4.map(r=>{const cN=parseFloat(r.concN)||0,q=parseFloat(r.quantitat)||0,s=parseFloat(r.superfici)||0,kN=cN&&q?Math.round(cN*q*100)/100:'',dn=kN&&s?Math.round(kN/s*100)/100:'';
      return[fn(r.parcelles),r.cultiu||'',r.superfici||'',r.dataFinal?fd(r.data)+'-'+fd(r.dataFinal):fd(r.data),r.tipus||'',r.nom||'',r.composicio||'',r.origen||'',r.concN||'',r.quantitat||'',kN,dn,r.fertirreg?'Sí':'No',r.aplicador||''];})
  ],[20,22,12,22,26,20,14,20,14,14,14,12,12,20]),'4-Adobats');

  const t10 = recs.filter(r=>r.type==='compra');
  XLSX.utils.book_append_sheet(wb, ws([
    ['Data','Producte','Qualificació','Quantitat (kg o L)','Proveïdor','Núm. albarà'],
    ...t10.map(r=>[fd(r.data),r.prod||'',r.qualificacio||'',r.quantitat||'',r.prov||'',r.albara||''])
  ],[14,28,14,16,24,16]),'10-Compres');

  const t11 = recs.filter(r=>r.type==='collita');
  XLSX.utils.book_append_sheet(wb, ws([
    ['Campanya','Producte','Qualificació','Data inici','Finques','Superfície (ha)','Venda (kg)','Autoconsum (kg)','Total (kg)','Rendiment (kg/ha)'],
    ...t11.map(r=>{const s=parseFloat(r.superfici)||0,tot=parseFloat(r.prodTotal)||0,rend=s&&tot?Math.round(tot/s):'';
      return[r.data?r.data.slice(0,4):'',r.cultiu||'',r.qualificacio||'',r.dataInici?fd(r.dataInici):fd(r.data),fn(r.parcelles),r.superfici||'',r.prodVenda||'',r.prodAutoconsum||'',tot||'',rend];})
  ],[10,24,12,12,22,12,12,14,12,14]),'11-Recolleccio');

  const t14 = recs.filter(r=>r.type==='venda');
  XLSX.utils.book_append_sheet(wb, ws([
    ['Data albarà','Núm. albarà','Producte','Quantitat (kg)','Finques/Lot','Qualificació','Client'],
    ...t14.map(r=>[fd(r.data),r.albara||'',r.prod||'',r.quantitat||'',(r.parcelles||[]).join(', '),r.qualificacio||'',r.client||''])
  ],[14,14,24,12,14,14,30]),'14-Venda');

  const t15 = recs.filter(r=>r.type==='altres');
  XLSX.utils.book_append_sheet(wb, ws([
    ['Data','Operari','Incidència / Observació'],
    ...t15.map(r=>[fd(r.data),r.user,r.descripcio||''])
  ],[14,16,60]),'15-Incidencies');
...
document.addEventListener('click', e => {
  document.querySelectorAll('.dd').forEach(d => { if (!d.parentElement?.parentElement?.contains(e.target)) d.style.display = 'none'; });
});
</script>
</body>
</html>
